...
#define D5 14 // SPI Bus MOSI

void setup() {
      pinMode(D7, OUTPUT); //Declare GPIO13 as output
      pinMode(D5, OUTPUT); //Declare GPIO13 as output
 
    Serial.begin(115200);

    WiFiManager wifiManager; //WiFimanager är ett smartare sätt att få kortet att anslua till ett wifi-nätverk.
    
    wifiManager.autoConnect("BoxLight");

    Serial.println("connected...yeey :)");

}

 String Lampname="404"; //Lampans namn
 float Temp= 0; //Färgtemperaturen
 float Ljus= 0; //Ljusstyrkan
 float kall = 0;
 float kall2 = 0;
 float varm = 0;
 int varmLamp = 0;
...
String url= "/grupp7/"+Lampname; //Urlen jag använder för att posta mina värden
...
String SendtoDB(String host){
  ...
  {
  String url= "/grupp7/"; //Urlen jag använder för att posta mina värden
  ...
  root["id"] = Lampname; //Skapar parameterna name och ger den värdet Vykort
  root["temp"] = Temp;
  root["ljus"] = Ljus;// Samma som ovan
...
void ConnecttoDB(String input){
  ...
  const char* host = "iot.abbindustrigymnasium.se"; //Adressen vi ska ansluta till. 7Laddaremygglustbil
...
void UpdatingLamp(){ //funktionen som uppdaterar lampans ljusstyrka och färgtemperatur.
varm = Temp/100; // gör om temperatur-värdet vi får från databasen till ett värde mellan 0-1.
varmLamp = Ljus * 10.24 * varm; // Bestämmer hur mycket de varma led-lamporna lyser. Tar ljusstyrkan multiplicerat med 10.24 för att få ett värde mellan 0-1024 och med varm som är till frö att ändra temperaturen.
analogWrite(D5, varmLamp); //skickar värdet som de varma led-lamporna ska ha till porten som är anluten till dem.
Serial.println(varmLamp);
kall = 100 - Temp; // detta blir det motsatta värdet till Varm, eftersom vi som sagt vill kunna ända färgtemperaturen. Deså högre procent destå varmare lampa.
kall2 = kall/100;
kallLamp = Ljus * 10.24 * kall2; // vi gör samma sak som vi gjorde för de varma lamporna men med variabeln för de kalla led-lamporna.
analogWrite(D7,kallLamp); //skickar värdet för de kalla led-stripsen till den port de är anslutna till.
Serial.println(kallLamp);

}

void loop() {
 ConnecttoDB("GET"); //kör funktionen som hämtar datan från databasen 
  UpdatingLamp(); //kör funktionen som uppdaterar ljusstyrkan och färgtemperaturen. 
}
